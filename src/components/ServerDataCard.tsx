...
import {
  ...
  CheckCircle2, Clock, Gamepad2, Zap, Wifi, ShieldCheck,
  ChevronRight, Server
} from 'lucide-react';
import type { MCServerResponse, WHOISData } from '@/types/minecraft';
import {
  analyzeRoutingQuality, analyzeSecurity, getSoftwareInfo,
  getPlatformType, getProtocolVersion, getOptimalLatency,
  haversineDistance, getUserLocation, copyToClipboard, renderObjectSmart
} from '@/lib/minecraft';
...
// --- Sub-Components for ServerDataCard ---

function ServerHeader({ serverData, onCopyIP, copied }: {
  serverData: MCServerResponse;
  onCopyIP: () => void;
  copied: boolean;
}) {
...
function ServerStatsGrid({ serverData, whoIsData, latency, measuringLatency }: {
  serverData: MCServerResponse;
  whoIsData: WHOISData | null;
  latency: number | null;
  measuringLatency: boolean;
...
function BotProtectionCard({ whoIsData, serverData }: { whoIsData: WHOISData; serverData: MCServerResponse }) {
...
function ConnectionAnalysisCard({ whoIsData, latency }: { whoIsData: WHOISData; latency: number | null }) {
...
function ServerInfoCard({ serverData }: { serverData: MCServerResponse }) {
...
function WhoisSection({ whoIsData, serverData }: { whoIsData: WHOISData; serverData: MCServerResponse }) {
  return (
...
function WhoisGridItem({ label, value, color, monospace, truncate, small }: {
  label: string;
  value: string;
  color: string;
  monospace?: boolean;
